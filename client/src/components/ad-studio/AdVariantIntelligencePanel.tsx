...
export default function AdVariantIntelligencePanel({ jobRecord, eliteAccess, onUseVariant }: Props) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-semibold tracking-[-0.02em] text-white/[0.97]">
            Scored ad angles from this run
          </h3>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/55">
            Each row is a distinct creative angle the engine evaluated. The highlighted card matches the
            render you are watching. Actions below are quick iterations—full regenerate endpoints ship next.
          </p>
        </div>
        {!eliteAccess ? (
          <p className="max-w-xs rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100/90">
            Elite unlocks the full Ad Studio pipeline: more variants, dual-render compares, and
            higher-performing ad outputs.
          </p>
        ) : null}
      </div>
