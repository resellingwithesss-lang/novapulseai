...
import { createAdminRouter } from "./ads.admin.routes"
...
function isAdminUser(role: string | undefined): boolean {
  return isAdminOrAboveRole(role)
}
...
/**
 * Admin-gated routes (rerender-from-variant, operator-review, lineage)
 * live in `./ads.admin.routes.ts`. We mount them here so the external
 * mount (`app.use("/api/ads", adsRoutes)`) and route ordering stay
 * identical. Helpers are injected to avoid a circular import cycle
 * between this module and the admin module.
 */
router.use(
  createAdminRouter({
